...
float ttdec_gpu_float(float* h_tensor, int* h_n_array, int* h_r_array,double *eps, float* singleError, float* singleComRatio ){
    ...
    assert(cudaSuccess == cudaStat1);
    assert(cudaSuccess == cudaStat2);
    // cout<<"数据A 传输时间：  "<<timer.seconds()<<endl;
    // printMatrix_Device(m,n,d_A,m,"A");
    // printMatrix_Device(10, 10, d_A, 10, "A1");
...
///计算rsvd_LU获得矩阵 [U SVT]
    ...
    // printf("\n***********************\n time1: %f \n*********************\n", timerSVD2.seconds());
    // printMatrix_Device(m,ks,d_U,m,"U");
    // printMatrix_Device(ks,n,d_A2,ks,"A2");
    // printMatrix_Device(10, 10, d_U, 10, "G1");
    // h_r_array[1] = k;
...
if(CalculateError){
    ...
    cublas_status = cublasGemmEx(cublasH,
                           ...
                           algo);
    assert(cublas_status == CUBLAS_STATUS_SUCCESS);
    // printMatrix_Device(10, 10, d_coreG2G3, 10, "G23");
