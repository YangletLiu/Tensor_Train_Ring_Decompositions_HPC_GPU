...
//nvcc rSVD.cu -o rsvd -lcurand -lcublas -lcusolver
using namespace std;
...
//转置B
    cublas_status = cublasSgeam(cublasH2,
    ...
    signed char jobu = 'S'; //  m columns of U
    signed char jobvt = 'S'; //  n columns of VT
...
//转置U 给UT
    ...
    if (cublasH2 ) cublasDestroy(cublasH2);   
...
void basicRandSVD(cusolverDnHandle_t cusolverH, cublasHandle_t cublasH, float *d_A, 
    const int m, const int n, const int k, const int s,const int p,
...
///获得随机正太分布矩阵B
    ...
    float *d_B, *d_C, *d_UT;
    cudaStat1 = cudaMalloc((void**)&d_B, sizeof(float)*n*ks);
    cudaStat2 = cudaMalloc((void**)&d_C, sizeof(float)*m*ks);
...
// step 2: copy A and B to device
    ...
    assert(cudaSuccess == cudaStat1);
    cublas_status = cublasSnrm2(cublasH, mn, d_A, 1, &before_ANorm2);
    assert(CUBLAS_STATUS_SUCCESS == cublas_status);
    // printMatrix_Device(m,n,d_A,m,"A");
    
    float RSE = 1; int k0 = 10;
    while(RSE > eps){ 
        k0 = k0 + step;
        cudaStat2 = cudaMalloc ((void**)&d_U  , sizeof(float) * m * (k0+s));
        cudaStat3 = cudaMalloc ((void**)&d_S  , sizeof(float) * (k0+s));
        cudaStat4 = cudaMalloc ((void**)&d_V  , sizeof(float) * n * (k0+s));
        basicRandSVD(cusolverH, cublasH, d_A, m, n, k0, s, p, d_U, d_S, d_V);

        float *d_VT = NULL, *d_SVT=NULL; int ks = k0+s;
        cudaStat1 = cudaMalloc ((void**)&d_VT  , sizeof(float) * n * ks);
        cudaStat2 = cudaMalloc ((void**)&d_SVT , sizeof(float) * k0 * n);
        assert(cudaSuccess == cudaStat1);
        assert(cudaSuccess == cudaStat2);
        ...
        // cublas_status = cublasSnrm2_v2(cublasH, mn, d_A, 1, &before);
        // assert(CUBLAS_STATUS_SUCCESS == cublas_status);
        ...
        cublas_status = cublasSgemm_v2(
            ...
            m);
        assert(CUBLAS_STATUS_SUCCESS == cublas_status);
        cublas_status = cublasSnrm2(cublasH, mn, d_A, 1, &re_ANorm2);
        assert(cudaSuccess == cublas_status);

        printf("re: %E , before : %E \n", re_ANorm2, before_ANorm2);
        RSE = re_ANorm2/before_ANorm2;
        printf("RSE = %E \n", RSE);
        if (d_VT   ) cudaFree(d_VT);
        if (d_SVT  ) cudaFree(d_SVT);
        if (RSE > eps){
            cudaStat1 = cudaMemcpy(d_A, h_A, sizeof(float) * mn, cudaMemcpyHostToDevice);
            assert(cudaSuccess == cudaStat1);
            if (d_U    ) cudaFree(d_U); d_U = NULL;
