...
// /usr/local/cuda/bin/nvcc -lcublas -lcusolver -lcurand -std=c++11 largeMatrixMulti.cu -o largeMatrixMulti
...
void smallargeMatrixMulti_once_device(cublasHandle_t cublasH, dt *d_A, dt *d_B, const int &ttrank, const int &m, const long long &n, dt *d_ATB){
    ...
    // else if(cublas_status == CUBLAS_STATUS_ARCH_MISMATCH)
    //     printf("CUBLAS_STATUS_ARCH_MISMATCH\n");
    // else if(cublas_status == CUBLAS_STATUS_NOT_SUPPORTED)
    //     printf("CUBLAS_STATUS_NOT_SUPPORTED\n");
    // else if(cublas_status == CUBLAS_STATUS_INVALID_VALUE)
    //     printf("CUBLAS_STATUS_INVALID_VALUE\n");
    // else if(cublas_status == CUBLAS_STATUS_EXECUTION_FAILED)
...
void smallargeMatrixMulti_slice_host_Tans(cublasHandle_t cublasH, dt *d_A, dt *h_B, const int &ttrank, const int &m,  const long long &n, dt *h_ATB, const int &p){
    long long slice = 0;
    if(n%p==0)
        slice = n/p;
    else
...
double calMSE(cublasHandle_t cublasH, dt *h_A, dt *h_G1, dt *h_G2, dt *h_G3, dt *h_G4, dt *h_G5, dt *h_G6, const int *ttRanks, const int *ttDemns, const int p){
    double normA = norm2HH(h_A, ttDemns[0]*ttDemns[1]* ttDemns[2]*ttDemns[3]* ttDemns[4]*ttDemns[5]);
    ...
    double normAr = norm2HH(h_A, ttDemns[0]*ttDemns[1]* ttDemns[2]*ttDemns[3]* ttDemns[4]*ttDemns[5]);
...
int main(){
    ...
    float time = ttdec_half_lu_device_6(h_A, ttRanks, ttDemns, calculateTimes, p, calError);
